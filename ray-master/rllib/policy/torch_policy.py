...
class LearningRateSchedule:
    ...
    @DeveloperAPI
    def __init__(self, lr, lr_schedule,  lr_vf=None, lr_vf_schedule=None):
        ...
        # for lr_vf_schedule
        if lr_vf_schedule is None:
            self.cur_lr_vf = lr_vf
        else:
            self._lr_vf_schedule = PiecewiseSchedule(
                lr_vf_schedule, outside_value=lr_vf_schedule[-1][-1], framework=None)
            self.cur_lr_vf = self._lr_vf_schedule.value(0)
    ...
    def on_global_var_update(self, global_vars):
        ...
        if self._lr_schedule:
            ...
            for opt in self._optimizers[0:1]:
                for p in opt.param_groups:
                    p["lr"] = self.cur_lr
        if self._lr_vf_schedule:
            self.cur_lr_vf = self._lr_vf_schedule.value(global_vars["timestep"])
