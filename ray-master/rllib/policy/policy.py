...
class Policy(metaclass=ABCMeta):
    ...
    def __init__(self, observation_space: gym.spaces.Space,
        ...
        self.tempenv = ParametricActionsEdgeGraph(config['env_config'])
    ...
    def _get_dummy_batch_from_view_requirements(
        ...
        obs = self.tempenv.reset()
        if not self.config["_disable_preprocessor_api"]:

            prep = DictFlatteningPreprocessor(self.tempenv.observation_space)
            #
            flatten_obs = prep.transform(obs)[None]
        ...
        for view_col, view_req in self.view_requirements.items():
            if not self.config["_disable_preprocessor_api"] and \
                ...
                # the graph input cannot be all zeros..
                if view_col == 'obs' or view_col == 'new_obs':
                    if isinstance(view_req.space, gym.spaces.Dict):

                        ret[view_col] = {s: np.tile(obs[s], ((batch_size), *np.ones_like(obs[s].shape)))
                                         for s in get_base_struct_from_space(view_req.space).keys()
                                         }
                    else:
                        ret[view_col] = np.tile(flatten_obs, (batch_size, 1))
                # # fill dummy logits with one, so kl won't be inf
                # elif view_col == 'action_mask' or view_col == 'a0_logits' or view_col == 'a1_logits' or view_col == 'a2_logits':
                #     ret[view_col] = get_dummy_batch_for_space(
                #         view_req.space,
                #         batch_size=batch_size,fill_value=1)
                # Range of indices on time-axis, e.g. "-50:-1".
                elif view_req.shift_from is not None:
                    ret[view_col] = get_dummy_batch_for_space(
                        view_req.space,
