...
#         # Worker.
#         self.workers.remote_workers()[0].remote.foreach_trainable_policy.remote(update)
...
def execution_plan(workers: WorkerSet,
    ...
    # train_op = rollouts.for_each(train_torch_distributed_allreduce)  # allreduce

    # Update KL after each round of training.
    # Update KL after each round of training.
    # train_op = train_op.for_each(UpdateKL(workers))
    # train_op = (
    #     rollouts.batch_across_shards()  # List[(grad_info, count)]
    #         .for_each(RecordStats()))
