...
def appo_surrogate_loss(policy: Policy, model: ModelV2,
    ...
    action_dist = dist_class(model_out, model, actions=train_batch[SampleBatch.ACTIONS])
    ...
    prev_action_dist = dist_class(behaviour_logits, policy.target_model, actions=train_batch[SampleBatch.ACTIONS])
    ...
    if policy.config["vtrace"]:
        ...
        action_kl = pre_dist_a0.kl(action_dist._a0_dist) + pre_dist_a1.kl(
            action_dist._a1_dist) + pre_dist_a2.kl(action_dist._a2_dist)  #
        action_kl = _make_time_major(action_kl)
        ...
        # The entropy loss.
        mean_entropy = reduce_mean_valid(
            _make_time_major(action_dist.entropy(actions)))
...
def add_values(policy, input_dict, state_batches, model, action_dist):
    ...
    emb_edge, emb_graphs_filtrated, action_mask, edge_index_mask, num_edges, edge_index = policy.model._curiosity_feature_net.obtain_graph_env()

    out = {}
    if not policy.config["vtrace"]:
        out[SampleBatch.VF_PREDS] = policy.model.value_function()

    out["edge_index_mask"] = edge_index_mask
    out["edge_index"] = edge_index
    out["emb_edge"] = emb_edge
    ...
    out["emb_graphs_filtrated"] = emb_graphs_filtrated

    out['a0_logits']= action_dist._a0_logits
    out['a1_logits']= action_dist._a1_logits
    out['a2_logits']=action_dist._a2_logits
    return out
